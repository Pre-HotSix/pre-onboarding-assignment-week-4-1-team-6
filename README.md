...
### 1. authenticated를 route에서 처리했습니다.  

💡 이유 : page내에서 authenticated를 처리하면 페이지마다 처리해야하는 번거로움 + 페이지 깜빡이는 현상이 발생하지만 route에서 직접 처리함으로써 해결했습니다.
...
💡 이유 : 보통 클라이언트 데이터 관리(상태관리)를 하기위해 리덕스를 사용하는데 이전 과제들을 하면서 느낀점이 클라이언트 데이터 와 서버 데이터를 리덕스라는 한 스토어에 모두 관리를 한다고 느꼈습니다. 그래서 서버 데이터관리를 react-query로 클라이언트 데이터관리를 recoil을 사용했습니다.

### 6. params로 페이지네이션, 검색을 관리해서 언제든 도메인에 접근 가능하게 했습니다.

💡 이유 : 어떤 링크를 다른 사용자에게 제공하기 쉽고, url에서 정보를 얻기 좋기 떄문에 params로 관리했습니다.
